...
use Monolog\Logger;
use Phalcon\DI\DI;
use Phalcon\Logger\Adapter\AdapterInterface as Adapter;
use Phalcon\Logger\Formatter\Line;
use Phalcon\Logger\Adapter\Syslog;
use Phalcon\Logger\Formatter\FormatterInterface;
use Phalcon\Logger\Logger as Multiple;
    ...
    public function __construct(DI $di, $aggregate = true, $formatter = 'line')
    {
        ...
        if ($di->has('log') && $log = $di->get('log')) {
            ...
            if ($log instanceof Adapter) {
                ...
                $multiple->addAdapter(clone $log);
                $multiple->addAdapter($debugbar_loger);
                /** @var DI\Service $service */
                $di->set('log', $multiple);
            } elseif($log instanceof Multiple) {
                $log->addAdapter($debugbar_loger);
