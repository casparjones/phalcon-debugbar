...
use Phalcon\Db\Result\PdoResult as Pdo;
use Phalcon\Forms\Element\ElementInterface as Element;
use Phalcon\Forms\Form;
use Phalcon\Mvc\Model;
use Phalcon\Mvc\Model\Resultset\Complex;
use Phalcon\Messages\Message;
use Phalcon\Messages\Messages as Group;
...
trait Formatter
{
    ...
    public function formatVars($vars)
    {
        ...
        if (is_object($vars)) {
            ...
            } elseif ($vars instanceof \StdClass) {
                @$vars = $prefix. $formatter->formatVar((array)$vars);
            } elseif ($vars instanceof \Traversable) {
                $result = array();
                foreach ($vars as $k => $v) {
                    $result[$k] = $v;
                }
